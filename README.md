...
- `update-smartplug-devices <netscanner.json>` — matches known TP-Link smart plugs by hostname (`hs103` → TP-Link HS103, `hs105` → TP-Link HS105, `ep25` → TP-Link EP-25, `tp25` → TP-Link TP-25); no external inventory needed. Sets `type` to `Smart Plug` and `model` accordingly.
...
- `update-kitchenlight-devices <netscanner.json>` — matches the kitchen light by MAC (`d4:a6:51:49:b3:72`), a Tuya-based device that reports a generic `wlan0` hostname; no external inventory needed. Sets `type` to `Smart Light`, `model` to `Tuya Smart Light`.
