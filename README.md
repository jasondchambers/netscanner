...
- `update-smartplug-devices <netscanner.json>` — matches known TP-Link smart plugs by hostname (`hs103` → TP-Link HS103, `hs105` → TP-Link HS105, `ep25` → TP-Link EP-25); no external inventory needed. Sets `type` to `Smart Plug` and `model` accordingly.
